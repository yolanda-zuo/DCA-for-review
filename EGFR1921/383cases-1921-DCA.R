rm(list = ls())
#radiomics <- read.csv(file = "E:\\test\\file_test_prediction.csv",header = TRUE, sep=",")

# tabular <- read.csv(file = "E:\\test\\tabular_result\\features_test_prediction_tabular.csv",header = TRUE, sep=",")
# tabular.head()
# image_file <- read.csv(file = "E:\\test\\image_file_result\\features_test_prediction_image_file.csv",header = TRUE, sep=",")
# path = "C:\\Users\\zuoya\\Desktop\\Code\\ipypb-fs\\08_Catboost\\Catboost\\767cases\\EGFR-01-FS\\RFE-RF-GB\\ex-test-for-auc-dca-lgb-MIRFELGB29-73.csv"
# path = "C:\\Users\\zuoya\\Desktop\\Code\\ipypb-fs\\08_Catboost\\Catboost\\767cases\\EGFR-01-FS\\Inter-test-for-auc-dca-MIRFELGB29-lgb73.csv"
path = path = "DCA-383CASES-1921INTER1.csv"
tabular_image_file <- read.csv(file = path,header = TRUE, sep=",")
#For Loop Example

# countries <- c('India', 'U K', 'Japan', 'U S A', 'Australia', 'China')
countries <- c("NB_ADB","KNN_RF","LR_RF","RFFI_LGBM","RF_RF","Stacking_RF","SVM_LGBM")

for (str in countries) {
  print(paste("Countries are:  ", str))
}
print("----This is Coming from Outside the For Loop---")
dca <- function(data, outcome, predictors, xstart=0.35, xstop=0.99, xby=0.01,
  ...
  if (graph==TRUE) {
    ...
    if(intervention==TRUE) {
      ...
      plot(x=nb$threshold, y=nb$all, type="l",col="grey", lwd=3 ,xlim=c(xstart, xstop), ylim=c(ymin, ymax), xlab="Threshold probability", ylab="Net benefit",cex.axis=1.5,cex.lab=1.5)
      # adding treat none option
      lines(x=nb$threshold, y=nb$none,lwd=3,col="black")
      #PLOTTING net benefit FOR EACH PREDICTOR
      for(m in 1:pred.n) {
        if (smooth==TRUE){
          lines(nb$threshold,data.matrix(nb[paste(predictors[m],"_sm",sep="")]),col=m+1,lty=1,lwd = 3)
        } else {
          lines(nb$threshold,data.matrix(nb[predictors[m]]),col=m+1,lty=1,lwd = 3)
        }
        # adding each model to the legend
        legendlabel <- c(legendlabel, predictors[m])
        #legendcolor <- c(legendcolor, m)
        legendcolor <- c(legendcolor, m+1)
        legendwidth <- c(3, 3)
        #legendpattern <- c(legendpattern, 2)
    ...
    # then add the legend
    legend("topright", legendlabel, cex=1.1, col=legendcolor, lwd=legendwidth, lty=legendpattern)
  ...
  print(results$net.benefit)
  return(results)

}


countries <- c("NB_ADB","KNN_RF","LR_RF","RFFI_LGBM","RF_RF","Stacking_RF","SVM_LGBM")  #1921

# x_labels = ["LogisticRegression", "NaiveBayesian",'RandomForest','Xgboost']  'rf',
# for (str in countries) {
#   S <- paste(str,'-ex1-1921', '.tiff', sep ="")
#   png(S, width = 465, height = 225, units='mm', res = 600)
#   # image_file or tabular
#   dca(data=tabular_image_file, outcome="label", predictors=c(str), smooth="TRUE")
#   dev.off()
# }
# png("revised-dca-383cases-internal1_1921-2023.jpeg", width = 225, height = 225, units='mm', res = 1000)
pdf("revised-dca-383cases-internal1_1921-2023.pdf")
dca(data=tabular_image_file, outcome="label", predictors=c("NB_ADB","KNN_RF","LR_RF","RFFI_LGBM","RF_RF","Stacking_RF","SVM_LGBM"), smooth="TRUE")
